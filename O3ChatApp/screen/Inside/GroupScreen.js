...
const GroupScreen = ({ user, navigation }) => {
  ...
  const fetchGroups = async () => {
    try {
      ...
      if (groupData.Items && groupData.Items.length > 0) {
        ...
        const userJoinedGroups = groupData.Items.filter(
          (group) =>
            group.members.find((member) => member === user.email) ||
            Object.keys(group.roles).includes(user.email)
        );
  ...
  <FlatList
    data={groups}
    renderItem={({ item }) => (
      <Pressable onPress={() => handleViewGroup(item)} style={styles.groupItem}>
        <Image style={styles.avatarImage} source={{ uri: item.avatarGroup }} />
        <View style={styles.groupInfo}>
          <Text style={styles.groupName}>{item.groupName}</Text>
          <Text
            style={styles.memberCount}
          >{`Thành viên: ${item.members.length}`}</Text>
        </View>
      </Pressable>
    )}
